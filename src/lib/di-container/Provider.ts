...
export const isValueProvider = <T>(provider: Provider<T>):
  provider is ValueProvider<T> => (provider as ValueProvider<T>).useValue !== undefined;
...
export const isClassProvider = <T>(provider: Provider<T>)
  : provider is ClassProvider<T> => (provider as ClassProvider<T>).useClass !== undefined;
